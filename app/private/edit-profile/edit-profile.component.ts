...
import {AbstractControl, Form, FormBuilder, FormControl, ValidatorFn} from "@angular/forms";
...
export class EditProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.store
      .dispatch(new SearchUsers(new BehaviorSubject('rysz')))
      .subscribe(value => {
        console.log(value);
      });
    ...
    if (userDetails.userId === undefined) {
      ...
      this.emailControl = new FormControl(userDetails.email,
        [
          emailValidator(loggedUser.email)]);
